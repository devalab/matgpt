...
def generate_single_sequence(model, initial_tokens, property_values, stoi, itos, max_new_tokens, temperature=1.2, top_p=0.9):
    ...
    try:
        ...
        if isinstance(property_values, ListConfig):
            property_values = list(property_values)
            
        # Handle both single values and lists of values
        if isinstance(property_values, (list, tuple)):
            for i, value in enumerate(property_values):
                x_num[0, i] = float(value)
        else:
            x_num[0, 0] = float(property_values)
...
def generate_sequences_with_property(cfg: DictConfig):
    ...
    try:
        ...
        # Convert cfg.sample.properties to a list if it's a ListConfig
        properties = list(cfg.sample.properties) if isinstance(cfg.sample.properties, ListConfig) else cfg.sample.properties

        # Check if properties is a list of lists or a single list
        if all(isinstance(item, (list, ListConfig)) for item in properties):
            property_list = properties
        else:
            # If it's a single list, wrap it in another list to make it a list of lists
            property_list = [properties]

        for property_value in property_list:
            # Ensure property_value is a list
            if not isinstance(property_value, (list, ListConfig)):
                property_value = [property_value]
            
            property_values = list(property_value)  # Convert to list if it's a ListConfig
...
def save_results(sequences, prop_values, cfg, property_names):
    try:
        # Ensure we're using the correct property names from the configuration
        property_names = cfg.training.selected_properties
        
        # Create dictionary dynamically based on number of properties
        data = {"Sequence": sequences}
        for i, prop_name in enumerate(property_names):
            data[prop_name] = []
            for values in prop_values:
                if len(values) > i:
                    data[prop_name].append(values[i])
                else:
                    data[prop_name].append(None)
        
        # Reorder columns to have properties first
        column_order = list(property_names) + ["Sequence"]
        output_df = pd.DataFrame(data)[column_order]
